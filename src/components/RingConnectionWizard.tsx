...
export default function RingConnectionWizard({ isOpen, onClose, onConnected }: RingConnectionWizardProps) {
  ...
  const startScan = async () => {
    ...
    if (!bleRingService.isAvailable()) {
      setError(`Подключение ${CORE_NAME} доступно в Android-приложении ${APP_NAME}.`);
      setStep('error');
      return;
    }
    ...
    try {
      ...
      if (granted.bluetooth !== 'granted') {
        throw new Error(`Разрешите ${APP_NAME} находить устройства поблизости в настройках Android.`);
      }
      ...
      <div className="relative mx-auto flex min-h-full w-full max-w-md flex-col px-6 py-7">
        <header className="z-10 flex items-center justify-between">
          <div className="flex items-center gap-2 text-[9px] font-mono uppercase tracking-[0.25em] text-white/35">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-300" /> {CORE_NAME}
          </div>
          <button
            onClick={onClose}
            disabled={isBusy}
        ...
        <AnimatePresence mode="wait">
          {step === 'intro' && (
            <motion.main key="intro" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="flex flex-1 flex-col justify-center py-6 text-center">
              ...
              <p className="mx-auto mt-3 max-w-xs text-xs leading-relaxed text-white/48">Синхронизируйте сон, пульс, ВСР, кислород, температуру и активность напрямую с {CORE_NAME}.</p>
          ...
          {(step === 'permission' || step === 'scanning') && (
            <motion.main key="scan" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="flex flex-1 flex-col items-center justify-center py-6 text-center">
              ...
              <p className="mt-2 max-w-xs text-xs leading-relaxed text-white/42">{step === 'permission' ? 'Android попросит доступ к устройствам поблизости и геолокации — она нужна только для поиска Bluetooth' : 'Поднесите кольцо к телефону на расстояние до одного метра. Если кольцо в оригинальном приложении — выйдите из него, иначе оно перестаёт вещать.'}</p>
          ...
          {step === 'devices' && (
            <motion.main key="devices" initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} className="flex flex-1 flex-col justify-center py-8">
              <p className="text-center text-[9px] font-mono uppercase tracking-[0.28em] text-emerald-200/60">{CORE_NAME}</p>
              <h2 className="mt-2 text-center text-2xl font-display">{devices.length > 0 ? 'Выберите кольцо' : 'Кольцо не найдено'}</h2>
              <div className="mt-8 flex flex-col gap-3">
                ...
                {devices.length === 0 && <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-6 text-center text-xs leading-relaxed text-white/45">Кольцо не найдено. Снимите его с зарядки, держите рядом с телефоном и закройте оригинальное приложение кольца — оно занимает Bluetooth.</div>}
              </div>
