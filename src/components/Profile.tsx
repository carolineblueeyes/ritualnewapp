...
export default function Profile({ onOpenSubscription, isSubscribed, onResetAll, onSyncMetrics, stats, healthSource, onRefreshHealth }: ProfileProps) {
  ...
  const connectSimulatedDevice = async (deviceAddress: string) => {
    ...
    if (bleRingService.isAvailable()) {
      ...
      if (ok) {
        const metrics = await bleRingService.getMetrics();
        setIsBleRingConnected(true);
        setConnectedRingName(deviceAddress);
        if (onSyncMetrics) onSyncMetrics({ hrv: metrics.hrv ?? 60, sleep: metrics.sleepHours ?? 7.5, activity: metrics.steps ?? 8000, pulse: metrics.restingHR ?? 60 });
      }
    } else {
      await new Promise(r => setTimeout(r, 1800));
      setIsBleRingConnected(true);
      setConnectedRingName(deviceAddress);
      if (onSyncMetrics) onSyncMetrics({ hrv: 66, sleep: 8.3, activity: 10240, pulse: 57 });
    }
    clearHealthCache();
    setConnectingDevice(null);
    setBleScanning(false);
  ...
  const handleDisconnectRing = async () => {
    ...
    clearHealthCache();
    if (onSyncMetrics) onSyncMetrics({ hrv: 54, sleep: 7.6, activity: 8420, pulse: 62 });
  };
  ...
  const startSyncSequence = async (type: 'healthkit' | 'healthconnect') => {
    ...
    try {
      ...
      if (onSyncMetrics) {
        onSyncMetrics({
          hrv: metrics.hrv ?? 54,
          sleep: metrics.sleepHours ?? 7.6,
          activity: metrics.steps ?? 8420,
          pulse: metrics.restingHR ?? 62,
        });
      }
