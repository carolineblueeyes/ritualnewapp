...
import { ShineBreakdown, getShineLabel, getShineColor } from '../services/health/shine';
...
export default function RitualDashboard({ practices, stats, onSelectPractice, shine, healthSource }: RitualDashboardProps) {
  ...
  // Generate historical data for 7, 30, or 90 days
  const getHistoricalAnalytics = (period: '7' | '30' | '90') => {
    ...
    // Stable hashing for historical fluctuation so it's consistent
    const getStableOffset = (s: string) => {
      let hash = 0;
      for (let j = 0; j < s.length; j++) {
        hash = s.charCodeAt(j) + ((hash << 5) - hash);
      }
      return (Math.abs(hash) % 7) - 3; // stable offset from -3 to +3
    };
    ...
    for (let i = daysCount - 1; i >= 0; i--) {
      ...
      // Calculate a realistic Shine score for past days
      let calculatedShine = 74; // default baseline
      if (practicesCount === 0) {
        calculatedShine = 72;
      } else {
        calculatedShine = Math.min(100, 72 + practicesCount * 7);
      }
      ...
      if (i === 0) {
        ...
        calculatedShine = Math.max(40, Math.min(100, calculatedShine));
      }
      ...
      list.push({
        ...
        label,
        practicesCount,
        shineScore: Math.round(calculatedShine),
        isToday: i === 0
      });
    }
  ...
  const shineWeekData = analytics7Days.map(d => d.shineScore);
            ...
            <main className="flex-1 w-full max-w-md mx-auto pt-5 pb-20 overflow-y-auto hide-scrollbar px-5">
              <AnimatePresence mode="wait">
                    ...
                    {[
                      ...
                      return (
                        <div key={metric.key} className="rounded-2xl border border-white/[0.04] bg-white/[0.02] overflow-hidden">
                          {/* Collapsed row */}
                          <div onClick={() => hasData && setExpandedMetric(isExpanded ? null : metric.key)} className={`flex items-center justify-between p-3.5 ${hasData ? 'cursor-pointer' : ''}`}>
                            ...
                            {isExpanded && hasData && (
                              ...
                              >
                                <div className="px-4 pt-3 pb-5 border-t border-white/[0.04]">
                                  ...
                                  {(() => {
                                    const getMetricHistory = (key: string, currentVal: number): number[] => {
                                      switch (key) {
                                        case 'sleep':
                                          return [7.0, 6.8, 7.5, 7.2, 7.9, 8.1, currentVal];
                                        case 'hrv':
                                          return [52, 54, 49, 58, 62, 60, currentVal];
                                        case 'hr':
                                          return [64, 62, 65, 61, 59, 57, currentVal];
                                        case 'activity':
                                          return [7800, 8400, 9100, 6400, 11200, 10800, currentVal];
                                        case 'resp':
                                          return [14.2, 13.9, 14.4, 13.8, 13.6, 13.7, currentVal];
                                        case 'oxygen':
                                          return [97, 98, 97, 99, 98, 98, currentVal];
                                        case 'temp':
                                          return [36.6, 36.5, 36.4, 36.6, 36.4, 36.5, currentVal];
                                        default:
                                          return [currentVal * 0.9, currentVal * 0.95, currentVal * 1.05, currentVal * 0.98, currentVal * 1.02, currentVal * 1.01, currentVal];
                                      }
                                    };
                                    ...
                                    const historyValues = getMetricHistory(metric.key, val!);
                ...
                {healthPage === 2 && (() => {
                  ...
                  const getShineAdvice = (score: number) => {
                      ...
                      <div className="flex flex-col gap-5 relative select-none">
                        ...
                        <div className="w-full relative py-2">
                          <svg className="w-full h-auto overflow-visible" viewBox={`0 0 ${width} ${height}`}>
                            ...
                            {analyticsPeriod !== '7' && (
                              <path
                                d={`M ${points[0].x} 95 ` + points.map(p => `L ${p.x} ${p.y}`).join(' ') + ` L ${points[points.length - 1].x} 95 Z`}
                        ...
                        <div className="grid grid-cols-2 gap-6 py-3 border-y border-white/[0.03]">
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-baseline gap-1.5">
                              <span className="text-2xl font-semibold font-mono text-[#E6B85C]">{currentSelectedDay.shineScore}%</span>
                              <span className="text-[9px] text-emerald-400/80 font-mono">тонус</span>
                            </div>
                          </div>
                        ...
                        <div className="flex flex-col gap-3 text-[11px] text-white/45 leading-relaxed font-normal">
                          ...
                          <div className="flex flex-col gap-2.5 pl-0.5">
                            <div className="flex gap-2">
                              ...
                              <div>
                                <strong className="text-white/70 font-normal">Индекс Сияния (золотая линия):</strong> физиологический тонус (0-100%). Каждая практика добавляет <span className="text-[#E6B85C]/90 font-medium">+7% за сессию</span>, поддерживается непрерывной серией дней и калибруется по HRV при синхронизации.
                              </div>
                            </div>
